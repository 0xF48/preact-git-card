...
var Component, Dev, Docs, EXAMPLES, Example, HEADER_TEXT, Header, Markdown, Markup, PROPS, ShaderBoxGradient, Slide, TITLE, h, render,
  boundMethodCheck = function(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new Error('Bound instance method accessed before binding'); } };
...
ShaderBoxGradient = __webpack_require__(8);

Markdown = __webpack_require__(9);
...
EXAMPLES = [['A Gtihub Repo Card', __webpack_require__(26), Example, 'https://github.com/arxii/preact-repo-card/blob/master/source/example.coffee?ts=4']];
...
PROPS = [['url', 'null', 'Full url to the repository. eg https://github.com/arxii/preact-slide'], ['dark', 'false', 'Set to `true` to enable the dark theme!']];

Header = class Header extends Component {
  constructor() {
    super();
    this.autoHideShowGradient = this.autoHideShowGradient.bind(this);
  ...
  componentDidMount() {
    this.gradient = ShaderBoxGradient(this._canvas, {
      seed: [1, 2, 3],
      speed: 1.0,
      fade: 1.0
    });
    // @gradient.shader.uniforms.speed.val = 0.5
    this.gradient.shader.uniforms.seed.val[0] = 151;
    this.gradient.shader.uniforms.seed.val[1] = 22;
    this.gradient.shader.uniforms.seed.val[2] = 0;
    // @gradient.shader.uniforms.fade.val = 0
    this.gradient.run();
    return window.addEventListener('scroll', this.autoHideShowGradient);
  }
  ...
  render() {
    return h('div', {
      ...
      vert: true,
      center: true
    }, 'npm i preact preact-repo-card'), h('a', {
      href: "https://github.com/arxii/preact-slide",
...
Dev = class Dev extends Component {
  constructor() {
    super();
  }
...
render(h(Dev), document.body, this.docs_el);
...
var Example, GitCard, h;

({h} = __webpack_require__(0));

GitCard = __webpack_require__(18);

module.exports = Example = class Example {
  render() {
    return h('div', {
      style: {
        width: 300,
      ...
      uri: 'ritzlgrmft/ionic-configuration-service'
    }));
  }
...
var Btn, Component, Header, I_CHART, I_GITHUB, I_MENU, PreactRepoCard, Slide, fetchData, h;
...
I_CHART = __webpack_require__(24);

I_MENU = __webpack_require__(25);

I_GITHUB = __webpack_require__(27);

fetchData = function(uri) {
  return new Promise(function(resolve, reject) {
    var dat;
    dat = [null, null];
    fetch('https://api.github.com/repos/' + uri).then(function(res) {
    ...
    return fetch('https://api.github.com/repos/' + uri + '/stats/participation').then(function(res) {
      return res.json().then(function(stats) {
        dat[1] = stats;
        if (dat[0]) {
          return resolve(dat);
        }
