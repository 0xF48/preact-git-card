...
var Btn, Component, Contributor, DIM, Graph, I_CHART, I_COURT, I_ERROR, I_EYE, I_GITHUB, I_LABEL, I_MENU, I_PEOPLE, I_PULL, I_SPLIT, I_STAR, Issue, LangBar, PUBLIC_, PreactRepoCard, Release, RepoContent, SingleStat, Slide, User, avatar, d3, fecha, fetchGithubRepoV3, fetchGithubRepoV4, h, rand, shuffle,
...
I_COURT = __webpack_require__(16);

I_ERROR = __webpack_require__(17);

I_PULL = __webpack_require__(18);
...
fetchGithubRepoV3 = function(user, repo) {
  return new Promise(async function(resolve, reject) {
    var data, lang, lines, lines_max, pre, ref, res;
    pre = 'https://api.github.com/repos/' + user + '/' + repo;
    res = (await Promise.all([fetch(pre), fetch(pre + '/stats/participation'), fetch(pre + '/releases'), fetch(pre + '/contributors'), fetch(pre + '/stargazers'), fetch(pre + '/languages'), fetch(pre + '/issues'), fetch(pre + '/commits')]));
    data = (await Promise.all(res.map(function(res) {
      return res.json();
    })));
    data[0].stats = data[1];
    data[0].releases = data[2];
    data[0].contributors = data[3];
    data[0].stargazers = shuffle(data[4]);
    data[0].languages = data[5];
    data[0].issues = data[6];
    data[0].commits = data[7];
    data[0].langs = [];
    lines_max = 0;
    ref = data[0].languages;
    for (lang in ref) {
      lines = ref[lang];
      lines_max += lines;
      data[0].langs.push({
        name: lang,
        color: languages[lang] && languages[lang].color,
        lines: lines
      });
    }
    data[0].lang_lines_max = lines_max;
    return resolve(data[0]);
  });
};

fetchGithubRepoV4 = function(user, repo) {
  var gql;
  gql = `{ repository(owner: "${user}", name: "${repo}") { isArchived diskUsage isLocked isFork labels(first:10){ edges{ node{ name } } } owner { avatarUrl login url } pullRequests(first:10,states:OPEN){ totalCount edges{ node{ createdAt merged author{ login } } } } releases(first:10){ totalCount edges{ node{ isPrerelease isDraft publishedAt url author{ login url avatarUrl } } } } forks(first: 10) { totalCount edges { node { pushedAt owner { avatarUrl login url } } } } hasIssuesEnabled languages(first: 10) { totalCount edges { node { color name } } } licenseInfo { name url } pushedAt stargazers(first: 30) { totalCount edges { node { name login url avatarUrl } } } issues(last: 20, states: OPEN) { totalCount edges { node { title url labels(first: 5) { edges { node { name } } } } } } } }`;
...
LangBar = class LangBar extends Component {
  render() {
    return h(Slide, {
      ...
      return h(Slide, {
        beta: lang.lines / this.props.lang_lines_max * 100,
        style: {
...
Issue = class Issue extends Component {
  ...
  render() {
    return h('div', {
      ...
      href: this.props.html_url,
      className: '-prc-release-name ' + (this.props.prerelease && '-prc-release-pre ' || '') + (this.props.recent && '-prc-release-recent' || '')
...
Release = class Release extends Component {
  constructor(props) {
    var published_date;
    super();
    published_date = new Date(props.published_at);
    this.state = {
...
RepoContent = class RepoContent extends Component {
  ...
  onIssueHover(issue) {
    boundMethodCheck(this, RepoContent);
    return this.setState({
      right_bar_alt: issue.title
    });
  }

  render() {
    var content, header, right_bar, stats;
    header = h(Slide, {
    ...
    // 	h 'img',src:I_MENU
      ...
      style: {
        background: languages[this.props.language].color
      }
    }), h('span', {
      className: '-prc-lang-count'
    }, this.props.language)), h(Slide, {
    ...
    if (this.state.right_bar === 'releases') {
      ...
      right_bar = this.props.issues.map((issue) => {
        user.onHover = this.onIssueHover;
        return h(Issue, issue);
      });
    }
    stats = h(Graph, this.props);
    content = h(Slide, {
      vert: false,
      ...
      href: this.props.html_url + '/' + this.state.right_bar
    }, this.state.right_bar), this.state.right_bar_alt && h('span', {
      className: '-prc-overlay-title-alt'
    }, '/' + this.state.right_bar_alt))), h(LangBar, this.props), h(Slide, {
      dim: DIM
    }, h(SingleStat, {
      icon: I_SPLIT,
      value: this.props.forks
    }), h(SingleStat, {
    ...
    // 	dim: DIM
      ...
      icon: I_ERROR,
      value: this.props.open_issues_count
    }), h(SingleStat, {
      onClick: this.showContributors,
      icon: I_PEOPLE,
      value: this.props.contributors.length
    }), h(SingleStat, {
      onClick: this.showReleases,
      icon: I_LABEL,
      value: this.props.releases[0] && this.props.releases[0].tag_name || 'no release'
...
PreactRepoCard = class PreactRepoCard extends Component {
  constructor(props) {
    ...
    fetchGithubRepoV4(props.user, props.repo).then((data) => {
      return this.setState({
        repo: data
      });
    });
...
module.exports = "data:image/svg+xml;base64,"
...
"use strict";
Object.defineProperty(__webpack_exports__, "__esModule", { value: true });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_d3_shape_src_line__ = __webpack_require__(20);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_d3_shape_src_curve_monotone__ = __webpack_require__(26);
...
"use strict";
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__src_path__ = __webpack_require__(22);
